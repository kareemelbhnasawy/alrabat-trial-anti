            ...
            <div className="grid grid-cols-1 lg:grid-cols-3">
                {ITEMS.map((item, idx) => (
                    <div key={idx} className="group relative flex flex-col">
                        ...
                        {/* Card Bottom */}
                        <Link to={item.link} className={`flex-1 p-10 ${item.color} ${item.textComp} flex flex-col justify-between transition-transform duration-300 hover:-translate-y-2`}>
                            <div>
                                <span className="block text-xs font-bold uppercase tracking-widest opacity-70 mb-4">Division 0{idx + 1}</span>
                            ...
                            <div className="mt-8 flex items-center font-bold text-sm tracking-wide">
                                EXPLORE <ArrowRight size={18} className="ml-2 transition-transform group-hover:translate-x-2" />
                            </div>
                        </Link>
                    </div>
                ))}
