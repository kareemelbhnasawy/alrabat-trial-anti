    ...
    <>
      ...
      <div className="pt-32 pb-16 bg-neutral-50">
        <div className="container-custom max-w-4xl">
          <Link
            to="/news"
            className="inline-flex items-center text-sm font-bold uppercase tracking-widest text-neutral-400 hover:text-primary mb-8"
          >
            <ArrowLeft className="mr-2" size={16} /> Back to News
          </Link>
          <div className="flex items-center space-x-4 mb-6 text-sm">
            <span className="bg-accent/10 text-accent px-3 py-1 rounded font-bold uppercase tracking-wider">
              {article.category}
            </span>
            ...
            <span className="text-neutral-500 flex items-center">
              <Clock size={14} className="mr-1" /> {article.readingTime}
            </span>
          </div>
            ...
            {article.bodyBlocks.map((block, idx) => {
              ...
              if (block.type === "quote")
                return (
                  <blockquote
                    key={idx}
                    className="border-l-4 border-accent pl-6 py-2 my-8 bg-neutral-50 rounded-r-lg"
                  >
                    <p className="text-xl italic font-display font-bold text-primary mb-2">
                      "{block.content}"
                    </p>
                    {block.author && (
                      <cite className="text-sm uppercase tracking-widest text-neutral-500 not-italic">
                        - {block.author}
                      </cite>
                    )}
                  </blockquote>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {article.gallery.map((img, idx) => (
                <img
                  ...
                  src={img}
                  alt={`Gallery ${idx}`}
                  className="w-full aspect-video object-cover rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                  onClick={() => {
