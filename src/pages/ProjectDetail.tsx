      ...
      if (p) {
        setRelated(
          projects
            .filter(
              (other) =>
                other.divisionSlugs?.some((slug) =>
                  p.divisionSlugs?.includes(slug)
                ) && other.id !== p.id
            )
            .slice(0, 3)
        );
    ...
    <>
      ...
      <div className="relative h-[70vh] w-full">
        ...
        <div className="container-custom relative z-20 h-full flex flex-col justify-end pb-20 text-white">
          <Link
            to="/projects"
            className="inline-flex items-center text-sm font-bold uppercase tracking-widest text-white/70 hover:text-white mb-6"
          >
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-wider rounded backdrop-blur-md"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex flex-col sm:flex-row gap-8 text-neutral-300">
            ...
            <div className="flex items-center">
              <Calendar size={20} className="mr-2 text-accent" /> {project.year}
            </div>
          </div>
        </div>
      </div>
              ...
              {project.gallery.map((item, i) => (
                ...
                >
                  <img
                    ...
                    className="slant-br shadow-sm hover:shadow-lg transition-shadow w-full h-64 object-cover"
                  />
                  {item.caption && (
