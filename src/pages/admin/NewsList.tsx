    ...
    return (
        <div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
                        ...
                        {news.map(item => (
                            <tr key={item.id} className="hover:bg-neutral-50 transition-colors">
                                ...
                                <td className="p-4 text-right space-x-2">
                                    <Link to={`/admin/news/${item.id}/edit`} className="inline-block p-2 text-neutral-400 hover:text-accent">
                                        <Edit2 size={16} />
                                    </Link>
                                    <button onClick={() => deleteNews(item.id)} className="inline-block p-2 text-neutral-400 hover:text-red-500">
                                        <Trash2 size={16} />
                                    </button>
                                </td>
                            </tr>
                ...
                {news.length === 0 && <div className="p-8 text-center text-neutral-400">No articles found.</div>}
            </div>
        </div>
    );
};
